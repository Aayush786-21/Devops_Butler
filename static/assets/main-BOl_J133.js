import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               */class We{constructor(){this.routes={"/":"projects","/deploy":"deploy","/history":"history","/repositories":"repositories","/domain":"domain","/env-vars":"env-vars","/settings":"settings","/logs":"logs"},this.currentPage=null,this.init()}init(){document.querySelectorAll(".nav-item").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const o=e.getAttribute("href");this.navigate(o)})}),window.addEventListener("popstate",()=>{this.loadPage(window.location.pathname)}),this.loadPage(window.location.pathname)}navigate(e){window.history.pushState({},"",e),this.loadPage(e)}loadPage(e){const t=this.routes[e]||"dashboard";if(t==="deploy"){i=null;const o=document.getElementById("projectSidebar");o&&(o.style.display="none");const s=document.getElementById("sidebar");s&&(s.style.display="block")}this.showPage(t),this.updateActiveNav(e),this.updatePageTitle(t),window.scrollTo({top:0,behavior:"smooth"})}showPage(e){document.querySelectorAll(".page").forEach(o=>{o.style.display="none"});const t=document.getElementById(`page-${e}`);if(t){if(t.style.display="block",e==="deploy"){const o=document.getElementById("deploy-status"),s=document.getElementById("deploy-success");o&&(o.textContent="",o.style.color=""),s&&(s.style.display="none")}}else{const o=document.getElementById("page-dashboard");o&&(o.style.display="block")}this.currentPage=e,this.loadPageData(e)}updateActiveNav(e){document.querySelectorAll(".nav-item").forEach(t=>{t.classList.remove("active"),t.getAttribute("href")===e&&t.classList.add("active")})}updatePageTitle(e){const t={projects:"Projects",deploy:"Deploy",history:"History",repositories:"Repositories",domain:"Domain","env-vars":"Environmental Variables",settings:"Settings",logs:"Logs"};document.getElementById("pageTitle").textContent=t[e]||"Dashboard"}loadPageData(e){switch(e){case"dashboard":ve();break;case"projects":O(),ve();break;case"history":je();break;case"repositories":Nt();break;case"domain":it();break;case"env-vars":Ce();break;case"settings":ze();break;case"logs":Jt();break}}}const A=new We;window.router=A;async function Ye(n){const e=await Je();if(!e)return;const t=P.find(a=>a.id==n),o=t?t.name:"this project";if(await Ke(o))try{console.log("Deleting project with token:",e.substring(0,20)+"...");const a=await fetch(`/projects/${n}`,{method:"DELETE",headers:{Authorization:`Bearer ${e}`}});if(console.log("Delete response status:",a.status),!a.ok){const r=await a.json().catch(()=>({}));if(console.error("Delete error response:",r),a.status===401){p("Session expired. Please login again.","error"),localStorage.removeItem("access_token"),localStorage.removeItem("authToken"),setTimeout(()=>{window.location.href="/login"},2e3);return}throw new Error(r.detail||"Failed to delete project")}P=P.filter(r=>r.id!=n),H=H.filter(r=>r.id!=n),J(H),p("Project deleted","success")}catch(a){console.error("Delete project error:",a),p(`Delete failed: ${a.message}`,"error")}}function Ke(n){return new Promise(e=>{const t=document.createElement("div");t.className="modal-overlay";const o=document.createElement("div");o.className="delete-confirmation-modal",o.innerHTML=`
    ...
    `,t.appendChild(o),document.body.appendChild(t);const s=o.querySelector(".cancel-btn"),a=o.querySelector(".delete-btn"),r=()=>{document.body.removeChild(t)};s.onclick=()=>{r(),e(!1)},a.onclick=()=>{r(),e(!0)},t.onclick=l=>{l.target===t&&(r(),e(!1))},a.focus()})}function Ze(n){try{const t=JSON.parse(atob(n.split(".")[1])).exp*1e3,o=Date.now();return t<o+5*60*1e3}catch{return!0}}async function Je(){const n=localStorage.getItem("access_token")||localStorage.getItem("authToken");return!n||Ze(n)?(p("Session expired. Please login again.","error"),localStorage.removeItem("access_token"),localStorage.removeItem("authToken"),setTimeout(()=>{window.location.href="/login"},2e3),null):n}let te=localStorage.getItem("access_token")||localStorage.getItem("authToken"),Ee=localStorage.getItem("username");document.addEventListener("DOMContentLoaded",()=>{pe(),window.location.pathname==="/login"||window.location.pathname.includes("login.html")||setTimeout(()=>{if(te&&Ee){Qe(),Xt();const e=document.getElementById("page-projects");e&&window.location.pathname==="/"&&(e.style.display="block")}},100)});function pe(){const n=document.getElementById("userSection"),e=document.getElementById("authButtons"),t=document.getElementById("logoutBtn"),o=document.getElementById("newDeployBtn");document.getElementById("userName"),document.getElementById("userEmail"),document.getElementById("userAvatar");const s=window.location.pathname==="/login"||window.location.pathname.includes("login.html");te&&Ee?(n.style.display="flex",e.style.display="none",t.style.display="block",o.style.display="block",qe(),O(),s&&(window.location.href="/")):(n.style.display="none",e.style.display="block",t.style.display="none",o.style.display="none",s||(window.location.href="/login"))}function Qe(){var a,r;document.getElementById("logoutBtn").addEventListener("click",()=>{localStorage.removeItem("access_token"),localStorage.removeItem("authToken"),localStorage.removeItem("username"),te=null,Ee=null,pe(),p("Logged out successfully","success"),A.navigate("/")});const n=document.getElementById("projectsSearch");n&&n.addEventListener("input",l=>{const d=l.target.value.toLowerCase();H=P.filter(c=>c.name.toLowerCase().includes(d)||c.repository&&c.repository.toLowerCase().includes(d)),J(H)});const e=document.getElementById("addProjectBtn");e&&e.addEventListener("click",()=>{window.router&&window.router.navigate("/deploy")});const t=document.getElementById("browseUploadLink");t&&t.addEventListener("click",l=>{l.preventDefault(),window.router&&window.router.navigate("/deploy")}),document.getElementById("newDeployBtn").addEventListener("click",()=>{i=null;const l=document.getElementById("projectSidebar");l&&(l.style.display="none");const d=document.getElementById("sidebar");d&&(d.style.display="block"),A.navigate("/deploy")});const o=document.getElementById("deployForm");o&&o.addEventListener("submit",St),ht();const s=document.getElementById("deploy-type");s&&s.addEventListener("change",l=>{const d=document.getElementById("single-repo-group"),c=document.getElementById("git-url-section"),m=document.getElementById("split-deploy-layout"),u=document.getElementById("git-url");l.target.value==="split"?(d&&(d.style.display="none"),c&&(c.style.display="none"),m&&(m.style.display="block"),u&&u.removeAttribute("required")):(d&&(d.style.display="block"),c&&(c.style.display="block"),m&&(m.style.display="none"),u&&u.setAttribute("required","required"))}),(a=document.getElementById("clearHistoryBtn"))==null||a.addEventListener("click",Ct),(r=document.getElementById("searchReposBtn"))==null||r.addEventListener("click",Re),Xe(),tt()}function Xe(){const n=document.querySelector(".search-input"),e=document.getElementById("spotlightModal"),t=document.getElementById("spotlightSearch"),o=document.getElementById("spotlightResults");!n||!e||!t||!o||(n.addEventListener("click",et),e.addEventListener("click",s=>{s.target===e&&fe()}),t.addEventListener("input",nt),o.addEventListener("click",at),document.addEventListener("keydown",s=>{s.key==="Escape"&&e.style.display!=="none"&&fe()}))}function et(){const n=document.getElementById("spotlightModal"),e=document.getElementById("spotlightSearch");n.style.display="flex",setTimeout(()=>{e.focus()},100)}function fe(){const n=document.getElementById("spotlightModal"),e=document.getElementById("spotlightSearch"),t=document.getElementById("spotlightResults");n.style.display="none",e.value="",t.innerHTML=`
  ...
  `}function tt(){const n=document.getElementById("domainWarningModal");if(!n||n.dataset.bound==="true")return;n.dataset.bound="true";const e=document.getElementById("domainModalCancelBtn"),t=document.getElementById("domainModalOpenConfigBtn"),o=()=>{n.style.display="none"};e&&e.addEventListener("click",o),t&&t.addEventListener("click",()=>{o(),Ie("domain-config"),Me()}),n.addEventListener("click",s=>{s.target===n&&o()})}function nt(n){const e=n.target.value.toLowerCase().trim(),t=document.getElementById("spotlightResults");if(!e){t.innerHTML=`
    ...
    `),t+="</div>",e.innerHTML=t}function at(n){const e=n.target.closest(".suggestion-item, .search-result-item");if(!e)return;const t=e.dataset.action,o=e.dataset.type,s=e.dataset.id;if(fe(),o==="project"&&s)ke(parseInt(s));else if(t)switch(t){case"new-deploy":window.router&&window.router.navigate("/deploy");break;case"repositories":window.router&&window.router.navigate("/repositories");break;case"history":window.router&&window.router.navigate("/history");break;case"domain":window.router&&window.router.navigate("/domain");break;case"settings":window.router&&window.router.navigate("/settings");break;case"projects":window.router&&window.router.navigate("/projects");break}}function it(){document.getElementById("page-domain")}let re=null;async function ve(){if(!(localStorage.getItem("access_token")||localStorage.getItem("authToken"))){const e=document.getElementById("vmStatusCard");e&&(e.style.display="none");return}try{const e=await fetch("/api/vm-status",{headers:N()});if(!e.ok){if(e.status===401){const o=document.getElementById("vmStatusCard");o&&(o.style.display="none");return}throw new Error("Failed to fetch VM status")}const t=await e.json();rt(t.vm_status,t.message),t.vm_status==="creating"?re||(re=setInterval(()=>{ve()},5e3)):re&&(clearInterval(re),re=null)}catch(e){console.error("Error loading VM status:",e);const t=document.getElementById("vmStatusCard");t&&(t.style.display="none")}}function rt(n,e){$e("vmStatusCard","vmStatusIcon","vmStatusSpinner","vmStatusMessage","vmStatusProgress","vmProgressFill","vmProgressText","dashboardActions",n,e),$e("vmStatusCardProjects","vmStatusIconProjects","vmStatusSpinnerProjects","vmStatusMessageProjects","vmStatusProgressProjects","vmProgressFillProjects","vmProgressTextProjects",null,n,e)}function $e(n,e,t,o,s,a,r,l,d,c){const m=document.getElementById(n),u=document.getElementById(e),y=document.getElementById(t),v=document.getElementById(o),f=document.getElementById(s),k=document.getElementById(a),S=document.getElementById(r),_=l?document.getElementById(l):null;m&&(m.style.display="block",v&&(v.textContent=c),d==="creating"?(y&&(y.textContent="⏳",y.className="vm-status-spinner spinning"),u&&(u.className="vm-status-icon vm-status-creating"),f&&(f.style.display="block",k&&(k.style.width="100%",k.className="vm-progress-fill vm-progress-indeterminate"),S&&(S.textContent="Creating your virtual machine... This may take 2-5 minutes.")),_&&(_.style.display="none")):d==="ready"?(y&&(y.textContent="✅",y.className="vm-status-spinner"),u&&(u.className="vm-status-icon vm-status-ready"),f&&(f.style.display="none"),_&&(_.style.display="grid")):d==="failed"&&(y&&(y.textContent="❌",y.className="vm-status-spinner"),u&&(u.className="vm-status-icon vm-status-failed"),f&&(f.style.display="none"),_&&(_.style.display="grid")))}function N(){const n={},e=localStorage.getItem("access_token")||localStorage.getItem("authToken");return e&&(n.Authorization=`Bearer ${e}`),n}let P=[],H=[];async function O(){const n=localStorage.getItem("access_token")||localStorage.getItem("authToken");if(!n){J([]);return}ct();try{const e=await fetch("/deployments",{headers:{Authorization:`Bearer ${n}`}});e.ok?(P=(await e.json()).map(o=>{var q;const s=o.git_url||"",a=s,r=s?(q=String(s).split("/").pop())==null?void 0:q.replace(/\.git$/,""):null,l=o.app_name||r||o.container_name||"Untitled Project",d=(o.status||"").toLowerCase();let c;d==="running"?c="running":d==="failed"||d==="error"?c="failed":c="imported";let m=!1,u="single",y=null,v=null;const f=String(o.git_url||""),k=f.startsWith("split::"),S=!o.parent_project_id&&!o.component_type;if(k){m=!0,u="split";try{const L=f.replace("split::","").split("|");L.length===2&&(y=L[0],v=L[1])}catch{}}else if(d==="imported_split")m=!0,u="split";else if(S&&f.includes("|")){m=!0,u="split";try{const L=f.split("|");L.length===2&&(y=L[0],v=L[1])}catch{}}const K=o.custom_domain&&o.domain_status&&o.domain_status.toLowerCase()==="active"?`https://${o.custom_domain}`:o.deployed_url||o.app_url||null;return{id:o.id,name:l,status:c,url:K,createdAt:o.created_at,updatedAt:o.updated_at,repository:a,repository_url:a,git_url:s,project_type:u,isSplit:m,frontend_url:y,backend_url:v,processPid:o.process_pid||null,port:o.port||null,startCommand:o.start_command||null,buildCommand:o.build_command||null,isRunning:o.is_running||!1,custom_domain:o.custom_domain||null,domain_status:o.domain_status||null,last_domain_sync:o.last_domain_sync||null}}),H=[...P],J(H)):J([])}catch(e){console.error("Error loading projects:",e),J([])}}function J(n){const e=document.getElementById("projectsGrid");if(e){if(n.length===0){e.innerHTML=`
    ...
    `;return}e.innerHTML=n.map(t=>{const o=t.status==="running"?"status-success":t.status==="failed"?"status-error":"status-info",s=t.status==="running"?"Running":t.status==="failed"?"Failed":"Imported",a=t.status==="running"?"🚀":"📦",r=t.updatedAt?X(t.updatedAt):"Recently";return`
      <div class="project-card" data-project-id="${t.id}" onclick="selectProject(${t.id})">
        ...
        <div class="project-info">
          <h3 class="project-name">${B(t.name)}</h3>
      ...
      `}).join("")}}async function lt(n){try{const e=P.find(o=>o.id===n);if(!e){p("Project not found","error");return}const t=Ae(e.url);if(!t){p("Project URL not available. Make sure the project is deployed.","error");return}window.open(t,"_blank"),p(`Opening ${e.name}...`,"info")}catch(e){console.error("Error opening project site:",e),p("Failed to open project site: "+e.message,"error")}}function ct(){const n=document.getElementById("projectsGrid");n&&(n.innerHTML=`
  ...
  `)}let i=null;function ke(n){O().then(()=>{const t=P.find(o=>o.id==n);if(!t){const o=H.find(s=>s.id==n);o&&(i=o,he(o));return}i=t,he(t)});const e=document.getElementById("page-project-config");e&&e.style.display!=="none"&&Be()}function he(n){const e=document.getElementById("sidebar");e&&(e.style.display="none");let t=document.getElementById("projectSidebar");t||(t=dt(),document.body.appendChild(t));const o=t.querySelector("#projectSidebarName");o&&(o.textContent=n.name);const s=t.querySelector("#projectSidebarId");s&&(s.textContent=n.id);const a=t.querySelector('a[data-project-page="status"]');a&&(n.project_type==="split"?a.style.display="flex":a.style.display="none"),t.style.display="block",document.getElementById("pageTitle").textContent=n.name,Fe(),Ie("deploy")}function dt(){const n=document.createElement("aside");return n.id="projectSidebar",n.className="sidebar project-sidebar",n.innerHTML=`
    ...
    `,document.getElementById("pageContent").appendChild(n));const e=document.getElementById("project-components-section");e&&(e.style.display="none"),Be();const t=document.getElementById("changeProjectNameBtn");t&&(t.onclick=()=>Ue()),n.style.display="block"}async function ut(){document.querySelectorAll(".page").forEach(e=>e.style.display="none");let n=document.getElementById("page-status");if(n||(n=document.createElement("div"),n.id="page-status",n.className="page",document.getElementById("pageContent").appendChild(n)),n.innerHTML="",i&&i.id)try{const e=await fetch(`/projects/${i.id}/components`,{headers:N()});if(e.ok){const o=(await e.json()).components||[],s=o.find(m=>m.component_type==="frontend"),a=o.find(m=>m.component_type==="backend"),r=s?s.status==="running"?"RUNNING":s.status.toUpperCase():"NOT DEPLOYED",l=a?a.status==="running"?"RUNNING":a.status.toUpperCase():"NOT DEPLOYED",d=(s==null?void 0:s.status)==="running"?"status-success":(s==null?void 0:s.status)==="failed"?"status-error":"status-info",c=(a==null?void 0:a.status)==="running"?"status-success":(a==null?void 0:a.status)==="failed"?"status-error":"status-info";n.innerHTML=`
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem;">
        ...
        <div class="project-card" style="margin: 0;">
          <div class="project-header">
            <div class="project-icon">💻</div>
            <div class="project-status ${k}">${v}</div>
          </div>
          <div class="project-info">
            <h3 class="project-name">Backend</h3>
            <div class="project-meta">
  ...
  `}async function me(){const n=document.getElementById("domainSuggestion"),e=document.getElementById("domainStatus"),t=document.getElementById("domainPrefix"),o=document.getElementById("platformDomain");if(!i||!i.id){e&&(e.innerHTML='<span class="status-muted">Select a project to configure its domain.</span>');return}const s=localStorage.getItem("access_token")||localStorage.getItem("authToken");if(!s){e&&(e.innerHTML='<span class="status-error">Please login to manage domains.</span>');return}n&&(n.textContent="Loading domain details...");try{const a=await fetch(`/projects/${i.id}/domain`,{headers:{Authorization:`Bearer ${s}`}});if(!a.ok)throw new Error(`Failed to load domain info (${a.status})`);const r=await a.json(),l=r.butler_domain||"butler.example.com";o&&(o.textContent=l);let d="";const c=r.custom_domain||r.suggested_domain||"";if(c)if(c.endsWith(l))d=c.slice(0,-(l.length+1));else{const m=c.split(".");m.length>0&&(d=m[0])}if(t){t.value=d;const m=r.suggested_domain&&r.suggested_domain.endsWith(l)?r.suggested_domain.slice(0,-(l.length+1)):"";t.placeholder=m||"project-slug or my.project"}if(n){const m=r.suggested_domain&&r.suggested_domain.endsWith(l)?r.suggested_domain.slice(0,-(l.length+1)):"";n.textContent=m?`Suggested: ${m} (you can use multiple labels like "my.project" or "portfolio.app"). Leave blank to remove. Domains become active after a successful deploy.`:`Enter a subdomain prefix (can be multiple labels like "my.project" or "portfolio"). The platform domain ${l} is fixed and cannot be changed.`}wt(e,r),i&&(i.custom_domain=r.custom_domain,i.domain_status=r.domain_status)}catch(a){console.error("Failed to load project domain info:",a),e&&(e.innerHTML='<span class="status-error">Could not load domain configuration.</span>')}}async function Et(){if(!i||!i.id){p("Select a project first","error");return}const n=localStorage.getItem("access_token")||localStorage.getItem("authToken");if(!n){p("Please login to manage domains","error");return}const e=document.getElementById("domainPrefix"),t=document.getElementById("platformDomain");let o=e?e.value.trim():"";const s=t?t.textContent.trim():"";let a="";if(o){if(o=o.trim().replace(/^\.+|\.+$/g,""),!o){p("Please enter a subdomain prefix.","error");return}if(!/^[a-z0-9.-]+$/i.test(o)){p("Subdomain prefix can only contain letters, numbers, hyphens, and dots.","error");return}if(o.includes("..")){p("Subdomain prefix cannot contain consecutive dots.","error");return}if(o.startsWith(".")||o.endsWith(".")){p("Subdomain prefix cannot start or end with a dot.","error");return}a=`${o}.${s}`}if(!a){if(!i.custom_domain){p("Enter a subdomain prefix to save, or leave blank to remove the domain.","info");return}if(!confirm("Remove the custom domain and revert to the default internal URL?"))return;await kt(),await me();return}const r={custom_domain:a,auto_generate:!1};try{const l=await fetch(`/projects/${i.id}/domain`,{method:"POST",headers:{Authorization:`Bearer ${n}`,"Content-Type":"application/json"},body:JSON.stringify(r)});if(!l.ok){const m=(await l.json().catch(()=>({}))).detail||"Failed to save domain";if(l.status===409){p(m,"error");const y=document.getElementById("domainStatus");y&&(y.innerHTML=`<span class="status-error">${B(m)}</span>`)}else throw new Error(m);return}const d=await l.json();p(`Domain saved: ${d.custom_domain}`,"success"),await me()}catch(l){console.error("Failed to save domain:",l),p(l.message||"Failed to save domain","error");const d=document.getElementById("domainStatus");d&&l.message&&(d.innerHTML=`<span class="status-error">${B(l.message)}</span>`)}}async function kt(){if(!i||!i.id){p("Select a project first","error");return}const n=localStorage.getItem("access_token")||localStorage.getItem("authToken");if(!n){p("Please login to manage domains","error");return}try{const e=await fetch(`/projects/${i.id}/domain`,{method:"DELETE",headers:{Authorization:`Bearer ${n}`}});if(!e.ok){const o=(await e.json().catch(()=>({}))).detail||"Failed to reset domain";throw new Error(o)}p("Domain removed. Project will use its internal URL.","success"),i&&(i.custom_domain=null,i.domain_status=null),await me()}catch(e){console.error("Failed to clear domain:",e),p(e.message||"Failed to clear domain","error")}}function It(n){ke(n)}async function Fe(){const n=localStorage.getItem("access_token")||localStorage.getItem("authToken");if(!n){console.log("No auth token found");return}try{const e=await fetch("/api/user/profile",{headers:{Authorization:`Bearer ${n}`}});if(e.ok){const t=await e.json(),o=document.getElementById("projectSidebar");if(o){const s=o.querySelector("#projectSidebarUserName"),a=o.querySelector("#projectSidebarUserEmail"),r=o.querySelector("#projectSidebarUserAvatar");if(s&&(s.textContent=t.display_name||t.username||"User"),a&&(a.textContent=t.email||"No email"),r)if(t.avatar_url){const l=new Image;l.onload=()=>{r.style.backgroundImage=`url(${t.avatar_url})`,r.style.backgroundSize="cover",r.style.backgroundPosition="center",r.textContent=""},l.onerror=()=>{r.style.backgroundImage="",r.textContent=(t.display_name||t.username||"U").charAt(0).toUpperCase()},l.src=t.avatar_url}else r.style.backgroundImage="",r.textContent=(t.display_name||t.username||"U").charAt(0).toUpperCase()}}else console.error("Failed to load user profile:",e.status)}catch(e){console.error("Error loading user profile:",e)}}function X(n){if(!n)return"Recently";const t=Date.now()-new Date(n).getTime(),o=Math.floor(t/6e4),s=Math.floor(t/36e5),a=Math.floor(t/864e5);if(o<1)return"Just now";if(o<60)return`${o}m ago`;if(s<24)return`${s}h ago`;if(a<7)return`${a}d ago`;const r=new Date(n);return r.toLocaleDateString("en-US",{month:"short",day:"numeric",year:r.getFullYear()!==new Date().getFullYear()?"numeric":void 0})}function Se(n){return n?new Date(n).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric",hour:"numeric",minute:"2-digit",hour12:!0,timeZone:"Asia/Kathmandu"}):"Unknown"}async function Bt(n,e){const t=document.getElementById("monorepo-section"),o=document.getElementById("frontend-folder"),s=document.getElementById("backend-folder");if(!(!t||!o||!s))try{const a=localStorage.getItem("access_token")||localStorage.getItem("authToken");if(!a)return;const r=n?`/api/detect-monorepo?project_id=${n}`:`/api/detect-monorepo?git_url=${encodeURIComponent(e)}`,l=await fetch(r,{headers:{Authorization:`Bearer ${a}`}});if(l.ok){const d=await l.json();if(d.is_monorepo){if(t.style.display="block",d.frontend_folder){o.innerHTML='<option value="">None (skip frontend)</option>';const c=document.createElement("option");c.value=d.frontend_folder,c.textContent=d.frontend_folder,c.selected=!0,o.appendChild(c)}else o.innerHTML='<option value="">None (skip frontend)</option>';if(d.backend_folder){s.innerHTML='<option value="">None (skip backend)</option>';const c=document.createElement("option");c.value=d.backend_folder,c.textContent=d.backend_folder,c.selected=!0,s.appendChild(c)}else s.innerHTML='<option value="">None (skip backend)</option>'}else t.style.display="none"}}catch(a){console.error("Error detecting monorepo structure:",a),t.style.display="none"}}async function Q(){await O();try{const n=await fetch("/deployments",{headers:N()});if(n.ok){const e=await n.json();document.getElementById("totalDeployments").textContent=e.length,document.getElementById("runningApps").textContent=e.filter(o=>o.status==="success").length;const t=document.getElementById("recentActivity");e.length>0?t.innerHTML=e.slice(0,5).map(o=>{const s=o.app_name||o.container_name||"Untitled Project";return`
    ...
    `;return}try{const n=await fetch("/deployments",{headers:N()});if(n.ok){const e=await n.json(),t=document.getElementById("historyTableBody");e.length===0?t.innerHTML=`
          <tr>
        ...
        `}).join("")}}catch(n){console.error("Error loading history:",n)}}async function Ct(){if(confirm("Are you sure you want to clear all deployment history?"))try{(await fetch("/deployments/clear",{method:"DELETE",headers:N()})).ok&&(p("History cleared successfully","success"),je())}catch{p("Error clearing history","error")}}async function xt(n){if(confirm(`Are you sure you want to destroy "${n}"?`))try{(await fetch(`/deployments/${n}`,{method:"DELETE",headers:N()})).ok?(p("Deployment destroyed successfully","success"),je(),jt()):p("Error destroying deployment","error")}catch{p("Network error","error")}}let D=[],Ne="";async function Re(){const n=document.getElementById("usernameSearch").value.trim();if(!n){p("Please enter a GitHub username","error");return}n!==Ne&&(D=[],Ne=n);const e=document.getElementById("repositoriesGrid");e.innerHTML='<div class="empty-state"><p>Loading repositories...</p></div>';try{const t=await fetch(`/api/repositories/${n}`),o=await t.json();t.ok&&o.repositories?o.repositories.length===0?e.innerHTML='<div class="empty-state"><p>No repositories found</p></div>':(e.innerHTML=o.repositories.map(s=>`
        ...
        `).join(""),be()):e.innerHTML=`<div class="empty-state"><p>${o.detail||"Error loading repositories"}</p></div>`}catch{e.innerHTML='<div class="empty-state"><p>Error loading repositories</p></div>'}}function _t(n,e){const t=D.findIndex(o=>o.url===n);if(t>=0)D.splice(t,1),be();else{if(D.length>=2){p("You can only select up to 2 repositories for a split repository","error");return}D.push({url:n,name:e}),D.length===2&&Lt(),be()}}function Lt(){const[n,e]=D,t=document.createElement("div");t.className="modal-overlay",t.id="splitImportModal";const o=document.createElement("div");o.className="modal-content enhanced",o.innerHTML=`
