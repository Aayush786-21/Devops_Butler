import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               */class Q{constructor(){this.routes={"/":"projects","/deploy":"deploy","/history":"history","/repositories":"repositories","/domain":"domain","/env-vars":"env-vars","/settings":"settings","/logs":"logs"},this.currentPage=null,this.init()}init(){document.querySelectorAll(".nav-item").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const n=e.getAttribute("href");this.navigate(n)})}),window.addEventListener("popstate",()=>{this.loadPage(window.location.pathname)}),this.loadPage(window.location.pathname)}navigate(e){window.history.pushState({},"",e),this.loadPage(e)}loadPage(e){const t=this.routes[e]||"dashboard";this.showPage(t),this.updateActiveNav(e),this.updatePageTitle(t),window.scrollTo({top:0,behavior:"smooth"})}showPage(e){document.querySelectorAll(".page").forEach(n=>{n.style.display="none"});const t=document.getElementById(`page-${e}`);if(t)t.style.display="block";else{const n=document.getElementById("page-dashboard");n&&(n.style.display="block")}this.currentPage=e,this.loadPageData(e)}updateActiveNav(e){document.querySelectorAll(".nav-item").forEach(t=>{t.classList.remove("active"),t.getAttribute("href")===e&&t.classList.add("active")})}updatePageTitle(e){const t={projects:"Projects",deploy:"Deploy",history:"History",repositories:"Repositories",domain:"Domain","env-vars":"Environmental Variables",settings:"Settings",logs:"Logs"};document.getElementById("pageTitle").textContent=t[e]||"Dashboard"}loadPageData(e){switch(e){case"projects":P();break;case"history":U();break;case"repositories":Se();break;case"domain":ce();break;case"env-vars":D();break;case"settings":_e();break;case"logs":Y();break}}}const L=new Q;window.router=L;async function X(o){const e=await te();if(!e)return;const t=g.find(r=>r.id==o),n=t?t.name:"this project";if(await Z(n))try{console.log("Deleting project with token:",e.substring(0,20)+"...");const r=await fetch(`/projects/${o}`,{method:"DELETE",headers:{Authorization:`Bearer ${e}`}});if(console.log("Delete response status:",r.status),!r.ok){const s=await r.json().catch(()=>({}));if(console.error("Delete error response:",s),r.status===401){d("Session expired. Please login again.","error"),localStorage.removeItem("access_token"),localStorage.removeItem("authToken"),setTimeout(()=>{window.location.href="/login"},2e3);return}throw new Error(s.detail||"Failed to delete project")}g=g.filter(s=>s.id!=o),b=b.filter(s=>s.id!=o),B(b),d("Project deleted","success")}catch(r){console.error("Delete project error:",r),d(`Delete failed: ${r.message}`,"error")}}function Z(o){return new Promise(e=>{const t=document.createElement("div");t.className="modal-overlay",t.style.cssText=`
    ...
    `,document.head.appendChild(a),t.appendChild(n),document.body.appendChild(t);const r=n.querySelector(".cancel-btn"),s=n.querySelector(".delete-btn"),c=()=>{document.body.removeChild(t),document.head.removeChild(a)};r.onclick=()=>{c(),e(!1)},s.onclick=()=>{c(),e(!0)},t.onclick=p=>{p.target===t&&(c(),e(!1))},s.focus()})}function ee(o){try{const t=JSON.parse(atob(o.split(".")[1])).exp*1e3,n=Date.now();return t<n+5*60*1e3}catch{return!0}}async function te(){const o=localStorage.getItem("access_token")||localStorage.getItem("authToken");return!o||ee(o)?(d("Session expired. Please login again.","error"),localStorage.removeItem("access_token"),localStorage.removeItem("authToken"),setTimeout(()=>{window.location.href="/login"},2e3),null):o}let j=localStorage.getItem("access_token")||localStorage.getItem("authToken"),M=localStorage.getItem("username");document.addEventListener("DOMContentLoaded",()=>{T(),window.location.pathname==="/login"||window.location.pathname.includes("login.html")||setTimeout(()=>{if(j&&M){oe(),Fe();const e=document.getElementById("page-projects");e&&window.location.pathname==="/"&&(e.style.display="block")}},100)});function T(){const o=document.getElementById("userSection"),e=document.getElementById("authButtons"),t=document.getElementById("logoutBtn"),n=document.getElementById("newDeployBtn");document.getElementById("userName"),document.getElementById("userEmail"),document.getElementById("userAvatar");const a=window.location.pathname==="/login"||window.location.pathname.includes("login.html");j&&M?(o.style.display="flex",e.style.display="none",t.style.display="block",n.style.display="block",K(),P(),a&&(window.location.href="/")):(o.style.display="none",e.style.display="block",t.style.display="none",n.style.display="none",a||(window.location.href="/login"))}function oe(){var r,s;document.getElementById("logoutBtn").addEventListener("click",()=>{localStorage.removeItem("access_token"),localStorage.removeItem("authToken"),localStorage.removeItem("username"),j=null,M=null,T(),d("Logged out successfully","success"),L.navigate("/")});const o=document.getElementById("projectsSearch");o&&o.addEventListener("input",c=>{const p=c.target.value.toLowerCase();b=g.filter(l=>l.name.toLowerCase().includes(p)||l.repository&&l.repository.toLowerCase().includes(p)),B(b)});const e=document.getElementById("addProjectBtn");e&&e.addEventListener("click",()=>{window.router&&window.router.navigate("/deploy")});const t=document.getElementById("browseUploadLink");t&&t.addEventListener("click",c=>{c.preventDefault(),window.router&&window.router.navigate("/deploy")}),document.getElementById("newDeployBtn").addEventListener("click",()=>{L.navigate("/deploy")});const n=document.getElementById("deployForm");n&&n.addEventListener("submit",he);const a=document.getElementById("deploy-type");a&&a.addEventListener("change",c=>{const p=document.getElementById("single-repo-group"),l=document.getElementById("split-repo-group"),u=document.getElementById("git-url");c.target.value==="split"?(p.style.display="none",l.style.display="block",u&&u.removeAttribute("required")):(p.style.display="block",l.style.display="none",u&&u.setAttribute("required","required"))}),(r=document.getElementById("clearHistoryBtn"))==null||r.addEventListener("click",be),(s=document.getElementById("searchReposBtn"))==null||s.addEventListener("click",ke),ne()}function ne(){const o=document.querySelector(".search-input"),e=document.getElementById("spotlightModal"),t=document.getElementById("spotlightSearch"),n=document.getElementById("spotlightResults");!o||!e||!t||!n||(o.addEventListener("click",ae),e.addEventListener("click",a=>{a.target===e&&A()}),t.addEventListener("input",se),n.addEventListener("click",le),document.addEventListener("keydown",a=>{a.key==="Escape"&&e.style.display!=="none"&&A()}))}function ae(){const o=document.getElementById("spotlightModal"),e=document.getElementById("spotlightSearch");o.style.display="flex",setTimeout(()=>{e.focus()},100)}function A(){const o=document.getElementById("spotlightModal"),e=document.getElementById("spotlightSearch"),t=document.getElementById("spotlightResults");o.style.display="none",e.value="",t.innerHTML=`
  ...
  `}function se(o){const e=o.target.value.toLowerCase().trim(),t=document.getElementById("spotlightResults");if(!e){t.innerHTML=`
    ...
    `;return}const n=re(e);ie(n)}function re(o){const e={projects:[],actions:[],navigation:[]};g&&g.length>0&&(e.projects=g.filter(a=>a.name.toLowerCase().includes(o)||a.repository&&a.repository.toLowerCase().includes(o)));const t=[{name:"New Deploy",action:"new-deploy",icon:"🚀"},{name:"Repositories",action:"repositories",icon:"📁"},{name:"History",action:"history",icon:"📜"},{name:"Settings",action:"settings",icon:"⚙️"},{name:"Domain",action:"domain",icon:"🌐"}];e.actions=t.filter(a=>a.name.toLowerCase().includes(o));const n=[{name:"Projects",action:"projects",icon:"📊"},{name:"Repositories",action:"repositories",icon:"📁"},{name:"History",action:"history",icon:"📜"},{name:"Domain",action:"domain",icon:"🌐"},{name:"Settings",action:"settings",icon:"⚙️"}];return e.navigation=n.filter(a=>a.name.toLowerCase().includes(o)),e}function ie(o){const e=document.getElementById("spotlightResults");let t='<div class="search-results">';o.projects.length>0&&(t+='<div class="search-category">',t+='<div class="search-category-title">Projects</div>',o.projects.forEach(n=>{const a=n.status==="running"?"🚀":"📦",r=n.status==="running"?"RUNNING":n.status==="failed"?"FAILED":"IMPORTED";t+=`
    ...
    `),t+="</div>",e.innerHTML=t}function le(o){const e=o.target.closest(".suggestion-item, .search-result-item");if(!e)return;const t=e.dataset.action,n=e.dataset.type,a=e.dataset.id;if(A(),n==="project"&&a)N(parseInt(a));else if(t)switch(t){case"new-deploy":window.router&&window.router.navigate("/deploy");break;case"repositories":window.router&&window.router.navigate("/repositories");break;case"history":window.router&&window.router.navigate("/history");break;case"domain":window.router&&window.router.navigate("/domain");break;case"settings":window.router&&window.router.navigate("/settings");break;case"projects":window.router&&window.router.navigate("/projects");break}}function ce(){document.getElementById("page-domain")}function x(){const o={},e=localStorage.getItem("access_token")||localStorage.getItem("authToken");return e&&(o.Authorization=`Bearer ${e}`),o}let g=[],b=[];async function P(){const o=localStorage.getItem("access_token")||localStorage.getItem("authToken");if(!o){B([]);return}pe();try{const e=await fetch("/deployments",{headers:{Authorization:`Bearer ${o}`}});e.ok?(g=(await e.json()).map(n=>{var l;const a=n.repository_url||n.git_url,r=a?(l=String(a).split("/").pop())==null?void 0:l.replace(/\.git$/,""):null,s=n.app_name||r||n.container_name||"Untitled Project",c=(n.status||"").toLowerCase();let p;return c==="running"?p="running":c==="failed"||c==="error"?p="failed":p="imported",{id:n.id,name:s,status:p,url:n.deployed_url||n.app_url,createdAt:n.created_at,updatedAt:n.updated_at,repository:a,repository_url:a,containerUptime:n.container_uptime||"Unknown",containerPorts:n.container_ports||"No ports",containerImage:n.container_image||"Unknown",containerStatus:n.container_status||"Unknown",isRunning:n.is_running||!1}}),b=[...g],B(b)):B([])}catch(e){console.error("Error loading projects:",e),B([])}}function B(o){const e=document.getElementById("projectsGrid");if(e){if(o.length===0){e.innerHTML=`
    ...
    `;return}e.innerHTML=o.map(t=>{const n=t.status==="running"?"status-success":t.status==="failed"?"status-error":"status-info",a=t.status==="running"?"Running":t.status==="failed"?"Failed":"Imported",r=t.status==="running"?"🚀":"📦",s=t.updatedAt?S(t.updatedAt):"Recently";return`
  ...
  `)}let i=null;function N(o){const e=g.find(n=>n.id==o);if(!e)return;i=e,F(e);const t=document.getElementById("page-project-config");t&&t.style.display!=="none"&&z()}function F(o){const e=document.getElementById("sidebar");e&&(e.style.display="none");let t=document.getElementById("projectSidebar");t||(t=ue(),document.body.appendChild(t));const n=t.querySelector("#projectSidebarName");n&&(n.textContent=o.name);const a=t.querySelector("#projectSidebarId");a&&(a.textContent=o.id),t.style.display="block",document.getElementById("pageTitle").textContent=o.name,O(),q("deploy")}function ue(){const o=document.createElement("aside");return o.id="projectSidebar",o.className="sidebar project-sidebar",o.innerHTML=`
  ...
  `,o.querySelectorAll(".project-nav-item").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const n=e.getAttribute("data-project-page");q(n),o.querySelectorAll(".project-nav-item").forEach(a=>a.classList.remove("active")),e.classList.add("active")})}),o}function me(){const o=document.getElementById("projectSidebar");o&&(o.style.display="none");const e=document.getElementById("sidebar");e&&(e.style.display="block"),i=null,document.getElementById("pageTitle").textContent="Projects",document.querySelectorAll(".page").forEach(n=>{n.style.display="none"});const t=document.getElementById("page-projects");t&&(t.style.display="block"),P()}function q(o){switch(document.querySelectorAll(".page").forEach(e=>{e.style.display="none"}),o){case"deploy":const e=document.getElementById("page-deploy");if(e&&(e.style.display="block",i&&i.repository_url)){const a=document.getElementById("git-url");a&&(a.value=i.repository_url)}document.getElementById("pageTitle").textContent="Deploy";break;case"configuration":ge();break;case"logs":const t=document.getElementById("page-logs");t&&(t.style.display="block",Y());break;case"domain-config":ye();break;case"env-vars":const n=document.getElementById("page-env-vars");n&&(n.style.display="block",D());break}}function ge(){let o=document.getElementById("page-project-config");o||(o=document.createElement("div"),o.id="page-project-config",o.className="page",o.innerHTML=`
      <div class="card">
        ...
        <div class="config-info-grid">
          ...
          <div class="config-row">
            ...
            <div class="config-value-container">
              <span class="config-value-text" id="projectConfigCreated">${i!=null&&i.createdAt?S(new Date(i.createdAt)):"Unknown"}</span>
            </div>
          </div>
          <div class="config-row">
            <div class="config-label">Last update:</div>
            <div class="config-value-container">
              <span class="config-value-text" id="projectConfigUpdated">${i!=null&&i.updatedAt?S(new Date(i.updatedAt)):"Unknown"}</span>
            </div>
          </div>
          <div class="config-row">
    ...
    `,document.getElementById("pageContent").appendChild(o)),z(),o.style.display="block"}function z(){if(!i)return;const o=document.getElementById("projectConfigName"),e=document.getElementById("projectConfigOwner"),t=document.getElementById("projectConfigId"),n=document.getElementById("projectConfigCreated"),a=document.getElementById("projectConfigUpdated"),r=document.getElementById("projectConfigPorts"),s=document.getElementById("projectConfigImage"),c=document.getElementById("projectConfigStatus");if(o&&(o.textContent=i.name||"Unknown"),e){const p=localStorage.getItem("username"),l=localStorage.getItem("displayName");e.textContent=l||p||"Unknown User"}t&&(t.textContent=i.id||"-"),n&&(n.textContent=i.createdAt?S(new Date(i.createdAt)):"Unknown"),a&&(a.textContent=i.updatedAt?S(new Date(i.updatedAt)):"Unknown"),r&&(r.textContent=i.containerPorts||"No ports"),s&&(s.textContent=i.containerImage||"Unknown"),c&&(c.textContent=i.containerStatus||"Unknown")}function ye(){let o=document.getElementById("page-project-domain");o||(o=document.createElement("div"),o.id="page-project-domain",o.className="page",o.innerHTML=`
      <div class="card">
    ...
    `,document.getElementById("pageContent").appendChild(o)),o.style.display="block"}function ve(o){N(o)}async function O(){const o=localStorage.getItem("access_token")||localStorage.getItem("authToken");if(!o){console.log("No auth token found");return}try{const e=await fetch("/api/user/profile",{headers:{Authorization:`Bearer ${o}`}});if(e.ok){const t=await e.json(),n=document.getElementById("projectSidebar");if(n){const a=n.querySelector("#projectSidebarUserName"),r=n.querySelector("#projectSidebarUserEmail"),s=n.querySelector("#projectSidebarUserAvatar");if(a&&(a.textContent=t.display_name||t.username||"User"),r&&(r.textContent=t.email||"No email"),s)if(t.avatar_url){const c=new Image;c.onload=()=>{s.style.backgroundImage=`url(${t.avatar_url})`,s.style.backgroundSize="cover",s.style.backgroundPosition="center",s.textContent=""},c.onerror=()=>{s.style.backgroundImage="",s.textContent=(t.display_name||t.username||"U").charAt(0).toUpperCase()},c.src=t.avatar_url}else s.style.backgroundImage="",s.textContent=(t.display_name||t.username||"U").charAt(0).toUpperCase()}}else console.error("Failed to load user profile:",e.status)}catch(e){console.error("Error loading user profile:",e)}}function S(o){if(!o)return"Recently";const t=Date.now()-new Date(o).getTime(),n=Math.floor(t/6e4),a=Math.floor(t/36e5),r=Math.floor(t/864e5);if(n<1)return"Just now";if(n<60)return`${n}m ago`;if(a<24)return`${a}h ago`;if(r<7)return`${r}d ago`;const s=new Date(o);return s.toLocaleDateString("en-US",{month:"short",day:"numeric",year:s.getFullYear()!==new Date().getFullYear()?"numeric":void 0})}async function fe(){await P();try{const o=await fetch("/deployments",{headers:x()});if(o.ok){const e=await o.json();document.getElementById("totalDeployments").textContent=e.length,document.getElementById("runningApps").textContent=e.filter(n=>n.status==="success").length;const t=document.getElementById("recentActivity");e.length>0?t.innerHTML=e.slice(0,5).map(n=>`
        ...
        `).join(""):t.innerHTML='<p style="text-align: center; color: var(--text-secondary); padding: 1rem 0;">No recent activity</p>'}}catch(o){console.error("Error loading dashboard:",o)}}async function he(o){var p,l,u,y;if(o.preventDefault(),!j){d("Please login to deploy applications","error"),window.location.href="/login";return}const e=o.target,t=((p=document.getElementById("deploy-type"))==null?void 0:p.value)||"single",n=(l=document.getElementById("git-url"))==null?void 0:l.value.trim(),a=(u=document.getElementById("frontend-url"))==null?void 0:u.value.trim(),r=(y=document.getElementById("backend-url"))==null?void 0:y.value.trim(),s=document.getElementById("deploy-status"),c=document.getElementById("deploy-success");if(c.style.display="none",s.textContent="",t==="split"){if(!a||!a.startsWith("http")||!r||!r.startsWith("http")){s.textContent="Please enter valid Frontend and Backend repository URLs",s.style.color="var(--error)";return}}else if(!n||!n.startsWith("http")){s.textContent="Please enter a valid Git repository URL",s.style.color="var(--error)";return}s.textContent="🚀 Deploying...",s.style.color="var(--primary)";try{const m=new FormData;t==="split"?(m.append("deploy_type","split"),m.append("frontend_url",a),m.append("backend_url",r)):(m.append("deploy_type","single"),m.append("git_url",n)),typeof i=="object"&&i&&i.id&&m.append("project_id",String(i.id));const C=await fetch("/deploy",{method:"POST",headers:x(),body:m}),k=await C.json();C.ok?(s.textContent="✅ Deployment successful!",s.style.color="var(--success)",k.deployed_url&&(c.style.display="block",document.getElementById("openAppBtn").href=k.deployed_url,document.getElementById("openAppBtn").textContent=`Open ${k.deployed_url}`),e.reset(),setTimeout(()=>{fe(),L.navigate("/applications")},2e3)):(s.textContent=`❌ Error: ${k.detail||"Deployment failed"}`,s.style.color="var(--error)")}catch{s.textContent="❌ Network error. Please try again.",s.style.color="var(--error)"}}async function we(){if(!j){document.getElementById("applicationsGrid").innerHTML=`
      <div class="empty-state">
        <p>Please login to view your applications</p>
        <a href="/login" class="btn-primary">Login</a>
      </div>
    `;return}try{const o=await fetch("/deployments",{headers:x()});if(o.ok){const e=await o.json(),t=document.getElementById("applicationsGrid");e.length===0?t.innerHTML=`
          <div class="empty-state">
        ...
        `).join("")}}catch(o){console.error("Error loading applications:",o)}}async function U(){if(!j){document.getElementById("historyTableBody").innerHTML=`
      <tr>
        <td colspan="5" class="empty-state">Please login to view deployment history</td>
      </tr>
    `;return}try{const o=await fetch("/deployments",{headers:x()});if(o.ok){const e=await o.json(),t=document.getElementById("historyTableBody");e.length===0?t.innerHTML=`
          <tr>
        ...
        `).join("")}}catch(o){console.error("Error loading history:",o)}}async function be(){if(confirm("Are you sure you want to clear all deployment history?"))try{(await fetch("/deployments/clear",{method:"DELETE",headers:x()})).ok&&(d("History cleared successfully","success"),U())}catch{d("Error clearing history","error")}}async function Ee(o){if(confirm(`Are you sure you want to destroy "${o}"?`))try{(await fetch(`/deployments/${o}`,{method:"DELETE",headers:x()})).ok?(d("Deployment destroyed successfully","success"),U(),we()):d("Error destroying deployment","error")}catch{d("Network error","error")}}async function ke(){const o=document.getElementById("usernameSearch").value.trim();if(!o){d("Please enter a GitHub username","error");return}const e=document.getElementById("repositoriesGrid");e.innerHTML='<div class="empty-state"><p>Loading repositories...</p></div>';try{const t=await fetch(`/api/repositories/${o}`),n=await t.json();t.ok&&n.repositories?n.repositories.length===0?e.innerHTML='<div class="empty-state"><p>No repositories found</p></div>':e.innerHTML=n.repositories.map(a=>`
    ...
    <div class="empty-state">
      <p>Search for a GitHub username to see their repositories</p>
        ...
        `)}else console.error("Failed to load environment variables")}catch(o){console.error("Error loading environment variables:",o)}R()}function R(){const o=document.getElementById("importEnvBtn"),e=document.getElementById("addEnvVarBtn"),t=document.getElementById("importEnvCard"),n=document.getElementById("cancelImportBtn"),a=document.getElementById("importEnvForm"),r=document.getElementById("projectSelector");r&&r.addEventListener("change",async s=>{$=s.target.value?parseInt(s.target.value):null,await D()}),o&&(o.onclick=()=>{t.style.display=t.style.display==="none"?"block":"none"}),n&&(n.onclick=()=>{t.style.display="none",document.getElementById("envFileInput").value=""}),e&&(e.onclick=()=>{Le()}),a&&(a.onsubmit=async s=>{s.preventDefault();const p=document.getElementById("envFileInput").files[0];p&&await xe(p)})}async function xe(o){try{const t=(await o.text()).split(`
`),n={};t.forEach(a=>{if(a=a.trim(),a&&!a.startsWith("#")&&a.includes("=")){const[r,...s]=a.split("="),c=s.join("=").trim().replace(/^["']|["']$/g,"");r.trim()&&(n[r.trim()]=c)}}),E={...E,...n},await H(),document.getElementById("importEnvCard").style.display="none",document.getElementById("envFileInput").value="",d("Environment variables imported successfully!","success")}catch(e){console.error("Error importing .env file:",e),d("Failed to import .env file","error")}}function Ce(){const o=document.getElementById("envVarsList");if(o){if(_.length===0){o.innerHTML=`
      <div class="empty-state">
